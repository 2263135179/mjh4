...
public class LoginActivity extends AppCompatActivity{
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       // showPassword=(CheckBox)findViewById(R.id.show_password);
        ...
        if(isSave){
            String account=pref.getString("account","");//获取名为"account"的键对应的字符串值，并将其赋值给字符串变量account。
        ...
        login.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                //用户名和密码正确
                if(account.equals("mjh")&&password.equals("111111")){
                    editor=pref.edit();//创建一个SharedPreferences.Editor对象editor，用于编辑SharedPreferences数据。

                    editor.putString("username",account);//使用editor.putString()方法将用户名存入名为"username"的键中

                    //“是否保存密码”勾选
                    if(savePassword.isChecked()){
                        editor.putBoolean("save_password",true);
                        editor.putString("account",account);//存键
...
//                    showOrHide(passwordEdit,true);//显示密码
//                }else{
//                    showOrHide(passwordEdit,false);//隐藏密码
//                }
//            }
//        });
    ...
    //显示或隐藏密码
    private void showOrHide(EditText passwordEdit, boolean isShow){//显示密码

        //记住光标开始的位置
        int pos = passwordEdit.getSelectionStart();
        if(isShow){
            passwordEdit.setTransformationMethod(HideReturnsTransformationMethod.getInstance());//显示密码
        }else{
